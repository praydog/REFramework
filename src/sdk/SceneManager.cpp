...
REManagedObject* get_current_scene() {
    ...
    const auto scene_manager_type = sdk::RETypeDB::get()->find_type("via.SceneManager");
    const auto current_scene_method = scene_manager_type->get_method("get_CurrentScene");

    if (current_scene_method == nullptr) {
        spdlog::error("Cannot find via.SceneManager.get_CurrentScene");
        return nullptr;
    }

    return current_scene_method->call<REManagedObject*>(sdk::get_thread_context(), scene_manager);
}

void set_timescale(REManagedObject* scene, float timescale) {
    const auto scene_type = (sdk::RETypeDefinition*)scene->info->classInfo;
